...
function Page() {
    ...
    const featuredProducts = useProductTabs(
        useMemo(() => [
            { id: 1, name: 'All',  nameA: 'جميع', categorySlug: null },
            { id: 2, name: 'Power Tools', nameA: 'أدوات كهربائية', categorySlug: 'power-tools' },
            { id: 3, name: 'Hand Tools', nameA: 'أدوات يدوية', categorySlug: 'hand-tools' },
            { id: 4, name: 'Plumbing',  nameA: 'السباكة', categorySlug: 'plumbing' },
        ], []),
    ...
    const newArrivalsLinks = useMemo(() => [
        { title: 'Wheel Covers', nameA: 'اغطية الاطارات',  url: url.products() },
        { title: 'Timing Belts', nameA: 'أحزمة التوقيت',  url: url.products() },
        { title: 'Oil Pans', nameA: 'أحواض الزيت',  url: url.products() },
        { title: 'Show All', nameA: 'عرض الكل',  url: url.products() },
    ], []);

    const latestPosts = useDeferredData(() => blogApi.getLatestPosts(8), []);
    const latestPostsLinks = useMemo(() => [
        { title: 'Special Offers', nameA: 'عروض خاصة',  url: url.blog() },
        { title: 'New Arrivals', nameA: 'القادمون الجدد',  url: url.blog() },
        { title: 'Reviews', nameA: 'المراجعات',  url: url.blog() },
    ], []);
